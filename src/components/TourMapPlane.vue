  ...
  methods: {
    ...
    setListener: function() {
      this.canvas.addEventListener('click', (event) => {
        ...
        if (department && this.departmentList[department].tourLoaded) {
          this.$router.push(`/panorama3D/${this.departmentList[department].id}`)
        } else if (department) {
          this.$message.info('无法获取科室UUID')
        }
      })
    ...
    getDepartmentTourList: function() {
      this.axios
          .get('/api/tour/listTours')
          .then(res => {
            console.log(res)
            for (let departmentTour of res.data.departmentTourList) {
              if (this.departmentList[departmentTour.departmentName]) {
                this.departmentList[departmentTour.departmentName].tourLoaded = true
                this.departmentList[departmentTour.departmentName].id = departmentTour.departmentId
              }
            }
